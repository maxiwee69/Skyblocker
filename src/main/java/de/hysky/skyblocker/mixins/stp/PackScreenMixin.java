...
public class PackScreenMixin {
	...
	private Stream<ResourcePackOrganizer.Pack> skyblocker$prioritizeSTPPacks(Stream<ResourcePackOrganizer.Pack> original, @Local(argsOnly = true) PackListWidget widget) {
		if (widget == this.availablePackList) {
			return original.sorted(Comparator.comparing(pack -> pack instanceof AbstractPackAccessor abstractPack && abstractPack.getProfile().getSkyblockerMetadata() != null && abstractPack.getProfile().getSkyblockerMetadata().stpSupported()).reversed());
		}
